...
public class HelloWorldRestControllerTest extends TestCase {
	...
	public void setUp() throws Exception {
		...
		this.mockMvc = MockMvcBuilders.standaloneSetup(helloWorldRestController).build();
		when(helloWorldService.getHelloWorld(anyString())).thenReturn(new HelloWorldBean("mockMvcTest"));
	}

	@Test
	public void testHello() throws Exception {
		this.mockMvc.perform(get("/rest/hello/name")).andDo(print()).andExpect(status().isOk())
				.andExpect(jsonPath("$.title").value("mockMvcTest"));
	}
}